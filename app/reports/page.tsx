...
export default function ReportsPage() {
  ...
  const [data, setData] = useState<{
    e: ReturnType<typeof listClinicalEncounters>;
    r: ReturnType<typeof listPrescriptions>;
  }>({ e: [], r: [] });

  useEffect(() => {
    setData({ e: listClinicalEncounters(), r: listPrescriptions() });
  }, [refresh]);
  ...
  const exportData = () => {
    const blob = new Blob(
      [JSON.stringify({ generatedAt: new Date().toISOString(), encounters: data.e, prescriptions: data.r }, null, 2)],
      { type: "application/json" },
    );
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = "dokter-jaga-report.json";
    anchor.click();
    ...
    <main style={page}>
      <header style={header}>
        ...
        <div>
          ...
          <h1 style={h1}>Laporan</h1>
        </div>
        <div style={actions}>
          <button type="button" onClick={() => setRefresh((x) => x + 1)} style={secondary}>
            <RefreshCw size={13} /> Refresh
          </button>
          <button type="button" onClick={exportData} style={primary}>
      ...
      <div style={wrap}>
        ...
        <section style={card}>
          {data.e.slice(0, 10).map((x) => (
            <div key={x.id} style={row}>
              <div style={avatar}>
                {(x.patient.name || "PS").slice(0, 2).toUpperCase()}
              </div>
              <div style={rowText}>
...
function M({ v, t }: { v: number; t: string }) {
  return (
    <div style={metric}>
      <b>{v}</b>
      <span>{t}</span>
